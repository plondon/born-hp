function draw() {
    ...
    for (var e = [], t = 0; t < points.length; t++)
      x = 100 * points[t].x, y = 100 * points[t].y, z = 100 * points[t].z, 0 != x || 0 != z ? (0 == z ? (a = Math.PI / 2 * (Math.abs(x) / x), depth = 1) : (a = Math.atan(x / z), depth = Math.abs(z) / z), a += 2 * Math.PI, x = Math.sin(a + Math.PI * camera.x / 180) * Math.sqrt(Math.pow(x, 2) + Math.pow(z, 2)) * depth, nz = x / Math.tan(a + Math.PI * camera.x / 180)) : nz = z, (0 != y || 0 != nz) && (0 == nz ? (a = Math.PI / 2 * (Math.abs(y) / y), depth = 1) : (a = Math.atan(y / nz), depth = Math.abs(nz) / nz), a += 2 * Math.PI, y = Math.sin(a + Math.PI * camera.y / 180) * Math.sqrt(Math.pow(y, 2) + Math.pow(nz, 2)) * depth), e[t] = {
        x: x + canvas.width / 2,
    ...
    for (var t = 0; t < e.length / gridSize; t++) {
        ...
        for (var i = 1; i < e.length / gridSize; i++) context.lineTo(e[t + i * gridSize].x, e[t + i * gridSize].y)
    }
    for (var t = 0; t < e.length / gridSize; t++) {
    ...
    context.stroke(), tick(e), requestAnimationFrame(draw)
}

function tick(e) {
    mouse.click && (mouse.click = !1, ripple(e)), mouse.drag ? (camera.x += mouse.deltaX / 10, mouse.deltaX = 0) : camera.x += mouse.deltaX / 10 - spin;
    mouse.drag ? (camera.y += mouse.deltaY / 20, mouse.deltaY = 0) : '';
...
for (var canvas = $("canvas").get(0), context = canvas.getContext("2d"), gridSize = 4 * Math.floor(Math.max($(window).width(), $(window).height()) / 75) + 1, points = [], ripples = [], camera = {
        x: 0,
        y: 0
    }, spin = 0, mouse = {
        x: 0,
